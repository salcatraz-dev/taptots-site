/**
 * TapTots auth.js v4
 * RECOVERY MODE ENABLED: guest access is allowed until Supabase/Stripe are configured.
 * ─────────────────────────────────────────────────────────────────
...
function ttIsAllowed() {
  if (!_ttConfigured) return true; // temporary recovery mode until backend is configured
...
function ttCurrentUser()   { return _ttConfigured ? _user : { id: 'guest', email: '', app_metadata: { provider: 'guest' } }; }
